...
| 07 | Facturation & Paiements | `modules/07-BILLING.md` | 🟢 Fait — `invoices`/`invoice_items`/`payments` (V12), agrégation consultations/hospit + couverture assurance, statut piloté par les paiements (EN_ATTENTE→PARTIEL→PAYE/ANNULE), encaissement multi-modes (Mobile Money manuel), reçu imprimable, tableau de bord + caisse du jour ; PDF binaire & webhook Mobile Money différés |
...
| 12 | Notifications (SMS/Email) | `modules/12-NOTIFICATIONS.md` | 🟢 Fait — file `notifications` (V13), `NotificationService` (enqueue + drain `@Scheduled`) + 3 canaux (SMS/Email/In-App, sender no-op en dev), inbox `/notifications` + badge nav, triggers câblés (RDV créé, résultat labo validé, alerte stock) ; intégration provider réelle (Africa's Talking / Spring Mail) différée |
| 13 | Rapports & Statistiques | `modules/13-REPORTS.md` | ✅ Terminé — couche `reports` en lecture seule (aucune table propre), agrège consultations/factures/paiements/labo/hospit/stock. Tableaux de bord admin (revenus jour/mois + variation, consultations, taux d'occupation des lits, top pathologies, alertes stock, répartition modes de paiement), médecin (mes consultations du jour, labo en attente de validation, RDV de la semaine) et pharmacie (réutilise `PharmacyDashboardDto`). Rapports : bilan financier mensuel, activité médicale, épidémiologie (top pathologies + prévalence par âge/sexe/service), liste des impayés, état du stock. Web `/reports` (dashboard + financial/activity/epidemiology/outstanding, vues print-friendly via `@media print`) + REST `/api/reports/*`. Export PDF/Excel binaire différé (pas de lib au pom — comme les bulletins) |
...
- [x] Module Facturation (factures, assurances, encaissement multi-modes, reçu imprimable, tableau de bord) — webhook Mobile Money & PDF binaire différés
