...
- **Fait** : `base.html` (chrome/sidebar/topbar/breadcrumb/module-tabs), `login.html`, `dashboard*`,
  ...
  **Rapports** (slice 10 : `reports/{dashboard,financial,activity,epidemiology,outstanding}`)
  ...
  et **Portail patient** (slice 12 : `portal/{layout,home,appointments,record,appointment-form,no-record}`).
  **79 / 88 templates** portent des clés `#{...}`.
...
- [x] **Slice 12 — Portail patient** (`portal/*`).
...
| 2026-06-29 | 12 — Portail patient | Les 6 templates `portal/{layout,home,appointments,record,appointment-form,no-record}` clés `#{}` (namespace `portal.{nav,home,col,appointments,record,form,norecord,flash}` + `portal.title_suffix`). `layout.html` : nav (Accueil/Mes rendez-vous/Mon dossier) via `#{portal.nav.*}`, `<title>` suffixe via `${title} + ' — ' + #{portal.title_suffix}`, logout réutilise `#{layout.logout}`. Titres de page passés en argument de fragment `layout(#{portal.x.title}, …)`. **Statuts dynamiques** `#{${'status.'+x}}` (badges RDV/consultations/factures) + type RDV via `${a.type != null} ? #{${'apptype.'+a.type}} : #{common.none}` ; `<select>` Type du form réutilise `#{apptype.CONSULTATION/SUIVI/TELECONSULTATION}`. Salutation paramétrée `#{portal.home.greeting(${patient.firstName})}` (« Bonjour {0} 👋 » — pas d'apostrophe → MessageFormat-safe). Réutilisation `common.{date,status,type,none,cancel}` ; cols Médecin/Motif/Diagnostic/N°/Analyse/Résultat/etc. → `portal.col.*`. Fallback no-record via `${message} ?: #{portal.norecord.message}`. 1 flash réel `portal.flash.requested` via `WebI18n` injecté dans `PortalWebController` (le flux d'erreur relaie `e.getMessage()` du service). Téléconsultation « Rejoindre » traduit. Bundles **1288 clés alignées** (3 langues, diff vide ; +58). Test `portal_i18n_fr_puis_en` (accueil FR « Mon dossier médical » → EN « My medical record »/« Upcoming appointments », dossier « My invoices »/« Lab results », form « Request an appointment »/« Send request ») ; l'ancien `PortalTest` (5 tests) reste vert. Suite complète verte (**186 tests**). |
