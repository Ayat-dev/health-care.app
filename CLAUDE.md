...
> **👉 NEXT UP:** Wave 2 — module 11 Maternity (06): `maternity_records` (one per patiente, FK→patients/users; gravidity/parity, LMP/EDD, delivery outcome + Apgar/newborn fields; status EN_COURS→ACCOUCHEE→CLOTURE) + `prenatal_visits` (CPN1–4: gestational age, BP, fetal HR, uterine height, edema/proteinuria, vaccines, next-visit). Master→detail shape (one record, many visits) — closest analog is consultation+items but the parent is a long-lived dossier, not a single encounter. `clinic_config.module_maternity` flag exists (V4, default FALSE). No new schema beyond DATABASE.md §7. Patient dossier can gain a "Maternité"/"Grossesse" tab (mirror the Séjours/Imagerie wiring in `PatientWebController`); only for female patients. MATERNITE department + rooms already exist (V3/V10).
