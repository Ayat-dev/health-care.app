...
> **👉 NEXT UP:** Wave 3 — module 13 Notifications (12): `notifications` table (DATABASE.md §11; NOT yet created — needs V13) + `NotificationService` que les modules amont appellent pour mettre en file (status EN_ATTENTE→ENVOYE/ECHEC), plus un `@Scheduled` qui draine la file (`@EnableScheduling` déjà actif via Pharmacy's `StockAlertService`). Canaux SMS (Africa's Talking)/Email (Spring Mail)/In-App; en dev, pas de clés API → un sender no-op/log qui marque ENVOYE suffit (intégration réelle différée). Triggers utiles déjà câblables : RDV créé, résultat labo VALIDE, facture impayée J+7, alerte stock (le cron stock existe déjà). In-App = lecture de la table + badge nav. Closest analog for the queue+scheduler shape is Pharmacy's `StockAlertService` (`@Scheduled` cron). Mind the recurring NBs: OSIV is OFF (map DTOs in-tx or fetch refs), no global `@RestControllerAdvice` (web flow catches; API guards surface bare 500), Thymeleaf `@{}`-not-in-`${}` + null-Boolean-in-SpEL + `formatDecimal 'WHITESPACE'` + layout-title typographic apostrophe gotchas.
