...
> **👉 NEXT UP:** Wave 3 — module 14 Reports & Statistics (13): reads across all tables (appointments, consultations, billing/payments, pharmacy, lab, hospitalization, maternity) to produce dashboards/exports. Only meaningful now that data exists across waves. Likely a `reports` pkg of read-only aggregate queries (no new tables — pure read layer) + a web dashboard (`/reports`) + REST (`/api/reports/...`). Reuse the dashboard-aggregate shape from Billing (`countByStatus`/`totalInvoiced`/`collectedBetween`) and Pharmacy. Mind the recurring NBs: OSIV is OFF (map DTOs in-tx or fetch refs), no global `@RestControllerAdvice` (web flow catches; API guards surface bare 500), Thymeleaf `@{}`-not-in-`${}` + null-Boolean-in-SpEL + `formatDecimal 'WHITESPACE'` + layout-title typographic apostrophe gotchas.
