...
> **👉 NEXT UP:** Wave 2 — module 7 Pharmacy & Stock (05): drugs + stock_items + dispensations. Dispensations consume prescriptions (the `prescription_items.drug_id` FK + `prescriptions.is_dispensed`/`dispensed_at` flags are already in place, waiting to be wired).
