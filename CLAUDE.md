...
> **👉 NEXT UP:** Wave 3 — module 12 Billing & Payments (07): `invoices` + `invoice_items` (FK→act_catalog) + `payments`, with `insurance_providers`/`act_catalog` already seeded (V4). This is the **aggregation** module — an invoice pulls billable lines from consultations/hospitalizations (FK columns `consultation_id`/`hospitalization_id` already in DATABASE.md §10) and applies insurance coverage % → patient_amount; status EN_ATTENTE→PARTIEL→PAYE (+ANNULE) driven by cumulative `payments`. Numbering FAC-YYYY-NNNNN off `clinic_config.invoice_prefix`. Closest analog for the master→items+payments shape is consultation+prescription_items, but here the "completion" is money-driven (sum of payments vs patient_amount), and lines can be sourced from multiple upstream modules. CAISSIER role exists for payment capture. Patient dossier **Facturation tab** is currently a placeholder (`patients/detail.html` `#tab-billing`) — wire it like the other tabs. Mind the recurring NBs: OSIV is OFF (fetch refs for DTO mapping), `#numbers.formatDecimal(...,'WHITESPACE',...)`, no global `@RestControllerAdvice` (web flow catches), Thymeleaf `@{}`-not-in-`${}` + null-Boolean-in-SpEL gotchas.
