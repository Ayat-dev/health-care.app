...
> **👉 NEXT UP:** All 14 functional modules are done (clinical build order complete). The remaining work is **non-functional + differentiation**, tracked step-by-step in **`docs/IMPROVEMENT-BACKLOG.md`** — THE source of truth for "what's next" from here on (this ledger stays the source of truth for the functional modules themselves). That backlog is prioritized P1→P4 with checkboxes + acceptance criteria; built from the 2026-06-19 market/security/UX gap analysis crossed with a code audit. **Start with P1 (production blockers):** P1.1 prod/dev Spring profiles + externalized secrets, P1.2 audit log, P1.3 anti-brute-force/lockout, P1.4 global `@RestControllerAdvice` (kills the bare-500-on-IllegalState gap noted across every Wave 2–3 module), P1.5 tests (currently 0). Pick one backlog item per session, `mvnd` to verify, tick its box + add a row to that file's progress journal, then `/clear`.
...
- [x] **10. Hospitalization & Beds** (08) — new pkg `hospitalization`: `Room` (chambre = 1+ lits via `capacity`; FK→departments; type STANDARD/PRIVE/SOINS_INTENSIFS/MATERNITE; `daily_rate`; `is_active`) + `Hospitalization` (FK→patients/rooms/users; admission_date/discharge_date; admission_reason; diagnosis_on_discharge; status ADMIS→TRANSFERE/SORTI/DECEDE). **Occupancy is derived, not stored** — count of ADMIS stays per room — so transfer/discharge frees a bed by flipping status (`countByRoomIdAndStatus`). `RoomService` (reference CRUD + toggle, no occupancy dep) + `HospitalizationService` (bedBoard grouped-by-department, availableRooms, admit/transfer/discharge/prefillAdmission; `ensureBedAvailable` rejects désactivée/complète; admit rejects already-ADMIS patient; **transfer = close current as TRANSFERE + open fresh ADMIS in new room**; cost = nights×daily_rate, ≥1 nuit). DTOs `RoomDto`(+occupiedCount/availableCount/occupants/isFull)/`HospitalizationDto`(+nights/daysSinceAdmission/estimatedCost)/`BedBoardDepartmentDto`(totals). REST `HospitalizationApiController` (`/api/beds`, `/api/rooms` ADMIN writes, `/api/hospitalizations` + PATCH transfer/discharge, writes `hasAnyRole('MEDECIN','INFIRMIER','ADMIN')`). Web `HospitalizationWebController` (`/hospitalization` plan des lits, `/list`, `/{id}` détail avec actions transfert/sortie inline, `/admit`, `/rooms` + room form/toggle). Templates `hospitalization/{beds,list,detail,admit-form,rooms,room-form}.html`; `.bed-grid`/`.bed-card` + `badge-ADMIS/TRANSFERE/SORTI/DECEDE` appended to app.css. `V10__hospitalization_tables.sql` (rooms + hospitalizations + 8 seeded rooms via dept-code subselect). Seed: 1 ADMIS séjour (p3, Ch.102, J+2) in `DataInitializer`. Patient dossier **Séjours tab wired** (`PatientWebController.hospitalizationService.findForPatient`); consultation detail gains "+ Admettre le patient" shortcut. **NBs:** (1) `/hospitalization/{id}` (Long) coexists with literal routes `/admit`,`/list`,`/rooms` — PathPattern matches literals first, no conflict. (2) DailyCareSheet (feuille de soins) in spec has NO table in DATABASE.md → deferred. (3) hospitalizations schema has no consultation_id (unlike lab/radio) → admit prefill from consultation just copies patient/doctor/motif, no FK stored. (4) facture finale différée au module Billing (detail shows prévisionnel only). (5) same bare-500-on-IllegalState API gap (no global `@RestControllerAdvice`). Verified end-to-end on H2: Flyway→v10, validate passes, all 6 pages render 200, **full lifecycle proven** (admit→302; double-admit rejected « déjà hospitalisé »; transfer flips bed 101→103 + stay TRANSFERE + new ADMIS; discharge SORTI + diagnosis persisted; bed board frees vacated beds), room CRUD + dup-number guard, `/api/beds`/`/api/hospitalizations`/`/api/rooms` via JWT.
- [x] **11. Maternity** (06) — new pkg `maternity`: `MaternityRecord` (FK→patients UNIQUE/users; one long-lived dossier per patiente — gravidity/parity, LMP/EDD, accouchement + nouveau-né Apgar/sex/weight; status EN_COURS→ACCOUCHEE→CLOTURE) + `PrenatalVisit` (CPN: visit_number, gestational age, poids, tension, RCF, hauteur utérine, présentation, œdème/protéinurie/fer/VAT, next-visit; cascade-all/orphan-removal via `addVisit` sync). Repos: `MaternityRecordRepository` (`findWithRefsById` + `findByPatientId` JOIN FETCH patient/doctor/visits→doctor — **single collection fetch, no MultipleBagFetch risk**; `existsByPatientId`; `search` by status), `PrenatalVisitRepository` (`findWithRefsById`, `findMaxVisitNumber` to suggest next CPN#). `MaternityService` (openRecord rejects dup dossier + non-F patient; **EDD auto = LMP+280j** if blank; recordDelivery →ACCOUCHEE; close →CLOTURE; addVisit/updateVisit with **gestational age auto-derived from LMP** when blank; visit doctor falls back to currentUser) + `MaternityRiskCalculator` (`@Component`: BP systolic>140 → RED, protéinurie → RED, prise de poids >2kg en ≤14j → ORANGE, <4 CPN à ≥36 SA → ORANGE; `gestationalAgeWeeks(lmp,ref)` null-safe; never throws). DTOs `MaternityRecordDto` (+ derived currentGestationalAgeWeeks/completedVisits/visits/alerts) / `PrenatalVisitDto` / `MaternityAlertDto` (level+message). REST `MaternityApiController` (`/api/maternity` + PATCH deliver/close + `/{id}/visits` GET/POST + PUT `/visits/{visitId}`; writes `hasAnyRole('MEDECIN','INFIRMIER','ADMIN')`). Web `MaternityWebController` (`/maternity` list, `/new` open, `/{id}` dossier 4-onglets, `/{id}/edit`, `/{id}/visits/new` + `/visits/{vid}/edit`, `/{id}/delivery`, `/{id}/close`). Templates `maternity/{list,form,record,visit-form,delivery-form}.html`; `badge-ACCOUCHEE/CLOTURE` appended to app.css. Nav link added (SOINS, 🤰). `V11__maternity_tables.sql` (maternity_records + prenatal_visits, patient_id UNIQUE). Seed: dossier EN_COURS for p1 (~28 SA, G2P1) + 2 CPN (CPN2 BP 150/95 + protéinurie → 2 RED alerts). Patient dossier **Maternité tab wired** — `PatientWebController` injects `maternityService.findForPatient` **only for gender F**; tab button + content also gated `gender == 'F'` (male p2 has no tab). Open-dossier shortcut from the tab (`/maternity/new?patientId=`). **NBs:** (1) **Thymeleaf can't nest `@{}` inside `${}`** — conditional form actions must put the ternary at attribute level: `th:action="${cond} ? @{a} : @{b}"`, NOT `${cond ? @{a} : @{b}}` (parse error). (2) **null-Boolean in SpEL `or`/`and` throws EL1001E** — row-highlight `(v.bpSystolic>140) or v.proteinuria` 500s when proteinuria is null (a CPN saved without ticking the box); guard `(v.proteinuria != null and v.proteinuria)`. Caught in verify (seeded visits had it set; a 3rd CPN with null surfaced it). (3) `clinic_config.module_maternity` flag (V4, default FALSE) is NOT gated on anywhere — nav/pages always available, consistent with lab/radiology. (4) layout-title apostrophe gotcha avoided (typographic `'`). (5) same bare-500-on-IllegalState API gap (no global `@RestControllerAdvice`). Verified end-to-end on H2: Flyway→v11, Hibernate validate passes, app boots + seeds load, all maternity pages + patient Maternité tab render 200 (session login), **risk calculator proven** (2 RED alerts from BP150+protéinurie), **full lifecycle proven** (add CPN3→302, deliver→ACCOUCHEE w/ newborn 3250g + Apgar 8/10·10/10, close→CLOTURE), dup-dossier guard (« existe déjà pour cette patiente »), `/api/maternity` list + detail (visits 1/2/3 + RED alert) via JWT.

### 💰 Wave 3 — Aggregation & cross-cutting
- [x] **12. Billing & Payments** (07) — the **aggregation** module. New pkg `billing`: `Invoice` (master; FK→patients + optional consultations/hospitalizations/insurance_providers; `invoice_number` FAC-YYYY-NNNNN off `clinic_config.invoice_prefix`; money split subtotal→insuranceAmount/patientAmount via coverage %; status **money-driven** EN_ATTENTE→PARTIEL→PAYE +ANNULE; `@Transient getBalanceDue`), `InvoiceItem` (FK→act_catalog optional, snapshots description/unit/total — qty×unit), `Payment` (FK→invoice; method ESPECES/ORANGE_MONEY/WAVE/MTN_MOMO/CARTE/VIREMENT/ASSURANCE, reference, cashier). Repos: `InvoiceRepository` (`findWithRefsById` fetches patient/insurance/createdBy/consultation/hospitalization + **only the `items` bag** — payments lazy-load in-tx to dodge MultipleBagFetchException; `search`/`findByPatient`; `findMaxSequence(prefix,start)` with dynamic substring offset since prefix is configurable; dashboard aggregates `countByStatus`/`totalInvoiced`/`totalCollected`/`collectedBetween`), **separate `PaymentRepository`** for the daily cash report derived query. `BillingService` (searchDto/findForPatient map in-tx; `prefillFromConsultation` adds a CONS_GEN line, `prefillFromHospitalization` adds a nights×daily_rate line; `create`/`update` [EN_ATTENTE only] rebuild items + `recompute` subtotal/insurance/patient + skip blank rows; `recordPayment` guards ANNULE/already-PAYE/overpay then `refreshStatus`; `cancel` appends `[ANNULÉE] motif`, blocks PAYE; `dashboard`/`dailyReport`). DTOs `InvoiceDto`/`InvoiceItemDto`/`PaymentDto`/`BillingDashboardDto`/`DailyCashReportDto`. REST `BillingApiController` (`/api/billing/{invoices,acts,insurance,reports/daily}`; create/update `hasAnyRole('CAISSIER','SECRETAIRE','ADMIN')`, pay `('CAISSIER','ADMIN')`, cancel ADMIN). Web `BillingWebController` (`/billing` dashboard+caisse du jour, `/billing/invoices` {list,detail,new,edit,pay,cancel}, `/billing/invoices/{id}/receipt` standalone print). Templates `billing/dashboard.html` + `billing/invoices/{list,detail,form,pay,receipt}.html` (form has dynamic line rows + act dropdown that JS-fills desc/price, inlines `acts` JSON; receipt mirrors the lab bulletin). Status badges (EN_ATTENTE/PARTIEL/PAYE/ANNULE) already in app.css — no CSS change. `V12__billing_tables.sql` (invoices/invoice_items/payments; act_catalog+insurance_providers already V4). Seed: `caissier/caissier123` (CAISSIER) + 2 invoices (FAC-00001 PARTIEL 11000/5000 for p1 from c1, FAC-00002 PAYE 5000 ORANGE_MONEY for p2). Patient dossier **Facturation tab wired** (`PatientWebController` injects `billingService.findForPatient`); consultation detail gains a "+ Créer une facture" shortcut. **NBs:** (1) **Hibernate 6 quirk:** a cross-entity `@Query("SELECT p FROM Payment p LEFT JOIN FETCH p.invoice…")` on a `JpaRepository<Invoice,…>` rendered as `SELECT new …Payment()` → `SyntaxException` at request time — moved it to a dedicated `PaymentRepository` derived query (`findByPaidAtGreaterThanEqualAndPaidAtLessThanOrderByPaidAtAsc`); no fetch joins needed since `dailyReport` maps in-tx. (2) `@{}`-not-in-`${}` honored: form action/cancel-link use attribute-level ternary `th:action="${cond} ? @{a} : @{b}"`. (3) overpay/already-paid/cancel guards fire (tx rollback) but API still returns bare 500 (same project-wide no-`@RestControllerAdvice` gap as pharmacy/lab/etc.); web flow catches + flashes. (4) money: `BigDecimal` + `setScale(2,HALF_UP)`, `formatDecimal(…,'WHITESPACE',…)` in templates. Verified end-to-end on H2: Flyway→v12, Hibernate validate passes, app boots + seeds load, all billing web pages + patient Facturation tab render 200 (session login), **coverage math proven** (80% on 7000 → ins 5600/patient 1400; 50% on 8000 → 4000/4000), **payment lifecycle proven** (600→PARTIEL bal 800, +800→PAYE bal 0), cancel→ANNULE w/ motif, web create 302 + `items[i]` binding + blank-row skip, daily report (count/total/byMethod) + acts/insurance via JWT.
- [x] **13. Notifications** (12) — the cross-cutting **outbound queue**. New pkg `notification`: `Notification` entity (FK→users optional/patients optional; type RAPPEL_RDV/RESULTAT_LABO/STOCK_ALERTE/FACTURE_IMPAYEE/SYSTEM; channel SMS/EMAIL/IN_APP; status EN_ATTENTE→ENVOYE/ECHEC; `scheduled_at` null=immédiat, `sent_at`, `error_message`, **`read_at`** for in-app unread). `NotificationRepository` (`findDue` = EN_ATTENTE & scheduled≤now; `search` userId/status/type; `findInboxForUser`/`countUnreadForUser` IN_APP only). `NotificationSender` interface + `LoggingNotificationSender` (dev no-op catch-all: logs + marks ENVOYE — real Africa's Talking/Spring Mail senders would declare a narrower `supports()` and be picked first). `NotificationService` (`@Transactional`): low-level `enqueue` (defaults recipient from patient.phone/user.username; never throws to caller), business triggers `notifyAppointmentCreated`/`notifyAppointmentReminder`/`notifyLabResultsValidated` (SMS patient + IN_APP médecin)/`notifyInvoiceOverdue`/`notifyStockAlert` (`enqueueInAppToRole('PHARMACIEN')`), `processQueue` drain (sender by channel → ENVOYE/ECHEC), in-app `inboxForCurrentUser`/`unreadCountForCurrentUser`/`markRead`/`markAllRead`, `sendTest` (ADMIN). `NotificationScheduler` (`@Component`, mirrors Pharmacy's `StockAlertService` cron shape): `0 * * * * *` drain · `0 0 18 * * *` J-1 RDV reminders (`AppointmentRepository.findPendingReminders` + flips `reminderSent`) · `0 30 8 * * *` invoice dunning J+7 (`InvoiceRepository.findOverdueUnpaid`). DTO `NotificationDto` (+`isRead()`). REST `NotificationApiController` (`/api/notifications` list+filters, `POST /test-sms` ADMIN). Web `NotificationWebController` (`/notifications` inbox, `/{id}/read`, `/read-all`). Template `notifications/list.html`. Nav: 🔔 link in PRINCIPAL with **red unread badge** (count surfaced by `GlobalModelAdvice.unreadNotifications` — now injects `NotificationService`, runs per page). `V13__notifications_tables.sql`. Triggers **wired live** into `AppointmentService.create` (SMS confirmation) + `LabService.validate` (SMS patient + IN_APP médecin) + `StockAlertService` (now enqueues STOCK_ALERTE in-app, was log-only). Seed: 4 notifications (in-app for dr.martin 1 unread/1 read, 1 sent SMS, 1 pending SMS that the per-minute cron drains on boot). **NBs:** (1) `read_at` column added to the schema (DATABASE.md §11 lacks it — needed for the in-app unread badge); documented deviation. (2) no Spring bean cycle: `NotificationService` depends only on repos+`ClinicConfigService`+`List<NotificationSender>`; the upstream services/advice depend on *it*. (3) `enqueue` swallows+logs so a notification failure never rolls back the caller's business tx (mirrors `StockAlertService`). (4) AppointmentDto `startTime` is `@JsonFormat yyyy-MM-dd'T'HH:mm` (no seconds) — API 400s on seconds; unrelated to this module but bit me in verify. (5) same bare-500-on-IllegalState API gap (no global `@RestControllerAdvice`); web flow catches. Verified end-to-end on H2: Flyway→v13, Hibernate validate passes, app boots + seeds load, **drain proven** (seeded pending SMS → ENVOYE on the first per-minute tick), inbox renders 200 + nav badge shows 1 unread + read-all clears it, `/api/notifications` list/filters + `/test-sms` (queue+immediate dispatch → ENVOYE) via JWT, **appointment-created trigger** (+1 RAPPEL_RDV SMS), **lab-validate trigger** (+2: SMS patient + IN_APP médecin).
- [x] **14. Reports & Statistics** (13) — the **pure read layer** (no new tables, no Flyway migration). New pkg `reports`: `ReportService` (`@Transactional(readOnly=true)`) aggregates across billing/consultation/lab/appointment/patient/hospitalization/room/pharmacy repos. **Key design point:** all entity→DTO mapping happens *inside* the readOnly service tx, so lazy assocs resolve even with OSIV off (the persistence context is open for the service call, just not in the view) — no fetch-join gymnastics needed for the dashboards; reused existing service mappers (`consultationService.toDto`/`labService.toDto`/`appointmentService.toDto`/`billingService.toDto`/`pharmacyService.dashboard`). Added aggregate queries to existing repos (no new repos): Consultation (`countBetween`, `findTopDiagnoses` group-by-diagnosis, `countByDepartmentBetween`, `countBySexBetween`, `findConsultationPatientBirthDates`→age-bucketed in Java to dodge H2/PG date-math differences, `findForDoctorBetween`), Payment (`sumByMethodBetween`), LabRequest (`countByStatus`, `findPendingValidationForDoctor`), Patient (`countActive`/`countNewBetween`), Hospitalization (`countByStatus`/`countAdmissionsBetween`), Room (`totalActiveBeds`), Appointment (`countBetween`). DTOs (in `dto` pkg): `LabelValueDto` (generic label+count/amount row), `AdminDashboardDto`, `DoctorDashboardDto`, `MonthlyFinancialReportDto`, `ActivityReportDto`, `EpidemiologyReportDto`, `OutstandingReportDto` (pharmacy dashboard reuses existing `PharmacyDashboardDto`). REST `ReportApiController` (`/api/reports/{dashboard/admin,dashboard/doctor,dashboard/pharmacy,daily-cash,monthly-financial,activity,epidemiology,outstanding,stock}`; per-endpoint `@PreAuthorize` — admin dash ADMIN-only, doctor/activity/epi MEDECIN+ADMIN, financial/cash CAISSIER+ADMIN, pharmacy/stock PHARMACIEN+ADMIN). Web `ReportWebController` (`/reports` admin dashboard hub + `/reports/{financial,activity,epidemiology,outstanding}`, all `@PreAuthorize`-gated). Templates `reports/{dashboard,financial,activity,epidemiology,outstanding}.html` (financial/activity/epi carry a month/year selector form). Nav: 📊 Rapports link in GESTION, `sec:authorize="hasAnyRole('ADMIN','MEDECIN')"`. Appended an `@media print` block to app.css (hides `.sidebar`/`.topbar`/`.no-print` — also benefits the existing bulletins/receipt). **NBs:** (1) the layout-title apostrophe gotcha bites in **`th:text` string literals too** — `'Aujourd''hui : '` 500'd at render (`Could not parse as expression`), and `layout('Rapport d''activité', …)` would too; both fixed with a typographic `'`. (2) export `format=pdf|excel` deliberately **not** implemented — no PDF/Excel lib at the pom (same deferral as lab/radiology bulletins + billing receipt); web views are print-optimized instead. (3) same bare-500-on-IllegalState API gap project-wide (no global `@RestControllerAdvice`) — but reports are read-only so this is largely moot. Verified end-to-end on H2 (no new migration; Flyway stays at v13, Hibernate validate passes, app boots): all 5 web pages render 200 (session login), admin dashboard shows real seed data (revenueMonth 10000 from the 2 seed payments, 2 consultations, beds 1/13 = 7.7%, top pathology « Gastrite aiguë », ESPECES/ORANGE_MONEY 5000 each), monthly-financial (invoiced 16000/collected 10000/outstanding 6000), epidemiology (age buckets + F/M sex split), outstanding (FAC-2026-00001, 6000 dû); **doctor dashboard** (dr.martin JWT) returns his week RDV mapped cleanly (no LazyInit), empty today-consultations/lab-pending as expected; **role gating proven** (MEDECIN → 403 on `/dashboard/admin`, 200 on `/activity`).

### 🚀 Wave 4 — Continuous
- [x] **15. Deployment** (15) — docker-compose.yml done · monitoring done (P4.3: Actuator + Prometheus/Grafana sous profil compose `monitoring`, sonde `/actuator/health` publique pour UptimeRobot) · **Web-first (2026-06-25)**: client desktop GELÉ (`desktop/FROZEN.md`, retiré du compose) ; **assistant de première installation `/setup`** (pkg `setup`: `SetupService` gate = 0 user + verrou volatile, `SetupGuardInterceptor` enregistré dans `WebConfig`, `SetupWebController` + `templates/setup/wizard.html`) crée admin+clinique+`clinic_config` sans `.env` — `ProdDataInitializer` reste l'escape hatch headless ; topologie 2-tiers + contrat de reprise desktop dans `docs/DEPLOYMENT.md` · TODO (ongoing): harden Flyway, backups, .env, audit trail
