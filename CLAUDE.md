...
> **👉 NEXT UP:** Wave 2 — module 9 Radiology (10): same "exam request → result" shape as Lab. Factor the shared pattern from the just-built `lab` package (`LabRequest`/`LabRequestItem`/`LabResult`, worklist, status flow EN_ATTENTE→EN_COURS→VALIDE→LIVRE). For radio the "result" is report text + image attachments rather than numeric values + abnormality ranges; reuse the `storage` pkg (`FileStorageService`) for image upload. No pre-seeded catalog table exists for radiology — add a `radiology_exam_catalog`-style reference or reuse `act_catalog`.
