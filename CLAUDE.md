...
> **👉 NEXT UP:** Wave 3 is **complete** (modules 12–14 done). No new clinical module remains in the build order. Remaining work is Wave 4 continuous — **module 15 Deployment hardening** (Flyway prod config, backups, `.env`, audit trail) — plus deferred lower-prio carry-overs: profile/change-password + account lockout (module 1); **binary PDF/Excel export** now wanted by lab/radiology bulletins, billing receipt, AND reports (module 14) once a PDF lib (iText/JasperReports + Apache POI) lands at the pom. Next session: pick module 15 hardening or knock out the PDF/Excel export carry-over (would light up `format=pdf|excel` on `/api/reports/*` + printable bulletins).
